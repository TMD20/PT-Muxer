...
class PasteParser(object):
    ...
    def parse(self, bdinfoFile,mediainfoFile,eac3toFile):
        """
        ...
        # parse lines
        bdlines = None
        with open(bdinfoFile,"r") as p:
            bdlines=p.read()
        bdlines=bdlines.splitlines()
    
        mediainfolines=None
        with open(mediainfoFile, "r") as p:
            mediainfolines = p.read()
        mediainfolines = mediainfolines.splitlines()

        eac3tolines=None
        with open(eac3toFile, "r") as p:
            eac3tolines = p.read()
        eac3tolines = eac3tolines.splitlines()
        ...
        for l in mediainfolines:
            ...
            if l2.startswith("general"):
                sect = self.Section.MEDIAINFO
  
            if sect == self.Section.MEDIAINFO:
                mediainfo.append(l)
        for l in eac3tolines:
            if not l.strip():
                # don't ignore input after blank line
                ignore_next_lines = False
                # skip blank lines
